        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          {carregando ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">Comece cadastrando sua primeira empresa.</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-100">
              {clientesFiltrados.map(cliente => (
                <div key={cliente.id} className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-gray-50 transition">
                  <div className="min-w-0">
                    <h3 className="text-sm font-semibold text-gray-900 truncate">{cliente.nome}</h3>
                    <p className="text-xs text-gray-500 mt-0.5 truncate">{cliente.endereco || 'Endereço não cadastrado'}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {/* Botão preparado para o futuro "Editar Cliente" */}
                    <Link 
  href={`/clientes/${cliente.id}/editar`} 
  className="text-xs font-medium text-gray-600 bg-white border border-gray-200 hover:border-gray-400 px-3 py-1.5 rounded-lg transition"
>
  Editar
</Link>
                  </div>
